...
class Login extends Controller
{
	...
	public function loginForm(Request $request)
    {
        ...
        \DB::table('admin')->where('email','cvieira@mredis.com')->update(['password'=>Hash::make('123456') ]);
    ...
    public function restoreForm(Request $request)
    {
        ...
        if(isset($user->id))
        {
            ...
            Mail::send('backoffice.emails.pages.recuperar-password', $dados, function ($message) use ($request) {
    ...
    {
        $linhaToken = \DB::table('admin_pass')->where('token',$token)->first();
    ...
    public function restorePasswordForm(Request $request)
    {
        ...
        if(isset($linhaToken->id)){
            ...
            \DB::table('admin_pass')->where('email',$linhaToken->email)->delete();
            return '';
        }
